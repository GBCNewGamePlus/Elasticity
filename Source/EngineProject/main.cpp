...
int CALLBACK WinMain(HINSTANCE hInstance, HINSTANCE previousInstance, PSTR cmdLine, INT nCmdShow)
{
	...
	{
		/*GameEngine::GameObject myGameObject1;
		engine->CreateGameObject(myGameObject1);
		myGameObject1.Attach(LuaScriptComponent("RunScript.lua"));*/
		//ClassObserver co;
		Dispatcher::GetInstance()->Subscribe(EventType::MouseEvent, &testFun);
		Dispatcher::GetInstance()->Subscribe(EventType::KeyboardEvent, &keyboardFun);
